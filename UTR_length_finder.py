...
def find_UTR_lengths(
    ...
    # Open output file in write mode here (or do so after the loop if you prefer)
        ...
        for polyA in db_polyA.features_of_type('polyA'):
            for interval in gene_intervals:
                ...
                if interval.start - 3 <= polyA.start <= interval.end + 3:
                    ...
                    # Record a normal UTR length if it's < 1000
                    ...
                    # If the length is in [-3, -2, -1, 0], store the polyA line
                    if current_UTR_length in [-3, -2, -1, 0]:
                        # Convert polyA feature object to string (like original GFF line)
                        polyA_line = str(polyA)
                        align_polyAs.append(polyA_line)

                    # Only do the comparison if both geneIdToCheck and UTRlengthToCheck are defined
